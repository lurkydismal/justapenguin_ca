...
function Editor( videoId ) {
    ...
    this.UpdateVideoDataButton =
        document.getElementById( "UpdateVideoDataButton" );
...
Editor.prototype.setupEvents = function() {
    this.PlayerLoadButton.addEventListener( "click",
                                            () => this.loadYoutubePlayer() );

    let TimeChangeButtons =
        document.getElementsByClassName( "timechange-button" );
    ...
    this.TimeRefreshButton.addEventListener( "click", () => {
        if ( this.YoutubePlayer && this.YoutubePlayer.getCurrentTime ) {
            this.onTimeChange(
                Math.floor( this.YoutubePlayer.getCurrentTime() ) );
        } else {
            this.onTimeChange( 0 );
        }
    } );
    ...
    document.addEventListener( "keydown", ( e ) => {
        if ( ( e.key === "Control" || e.key === "Shift" ) && e.shiftKey &&
             e.ctrlKey ) {
            if ( this.YoutubePlayer && this.YoutubePlayer.getCurrentTime ) {
                this.onTimeChange(
                    Math.floor( this.YoutubePlayer.getCurrentTime() ) );
...
Editor.prototype.updateMatchList = function( vid ) {
    ...
    fetch( "/videoMatches?vid=" + vid ).then( ( r ) => {
        r.json().then( json => {
            ...
            for ( let btn of editButtons ) {
                btn.onclick = () => {
                    this.editButtonPress(
                        btn, parseInt( btn.dataset.idx ), btn.dataset.p1char,
                        btn.dataset.p1name, btn.dataset.p2char,
                        btn.dataset.p2name, btn.dataset.winner,
...
Editor.prototype.editButtonPress = function( btn, idx, p1char, p1name, p2char,
                                             p2name, winner, time, seek ) {
...
Editor.prototype.loadYoutubePlayer = function() {
    ...
    this.YoutubePlayer = new YT.Player( "YoutubePlayer", {
        ...
        rel : 0,
        videoId : id,
        events : { onReady : () => { this.YoutubePlayer.setVolume( 10 ); } }
    } );
