document.addEventListener( "DOMContentLoaded", () => {
    ...
    for ( let btn of reportButtons ) {
        btn.addEventListener( "click", () => {
            ...
            if ( !answer || answer.length === 0 ) {
                return alert(
                    "Gotta give a reason for reporting something, pal." );
            }

            sendReport( btn.dataset.vid, btn.dataset.idx, btn.dataset.time,
                        answer );
        } );
    }
    ...
    function sendReport( vid, idx, time, reason ) {
        fetch( "/report", {
            ...
            body : JSON.stringify( { vid, idx, time, reason } )
        } ).then( () => { alert( "Your report has been logged. Thanks!" ); } );
    }
} );