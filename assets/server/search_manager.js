...
function matchTest( mData, sq ) {
    ...
    if ( p1set ) {
        if ( p2set ) {
            let p1 = playerMatches( mData.p1char, mData.p1name, sq.p1char,
                                    sq.p1name );
            let p2 = playerMatches( mData.p2char, mData.p2name, sq.p2char,
                                    sq.p2name );
            // Swapped:
            let p1asp2 = playerMatches( mData.p2char, mData.p2name, sq.p1char,
                                        sq.p1name );
            let p2asp1 = playerMatches( mData.p1char, mData.p1name, sq.p2char,
                                        sq.p2name );
            ...
            if ( p1 && p2 ) {
                ...
                if ( p1Char !== p2Char ) {
                    ...
                    if ( sq.winner === "1" && mData.winner !== "2" ) {
                        // We are searching for p1 winner, but in reality we
                        // want p2 to be a winner
                        return false;
                    } else if ( sq.winner === "2" && mData.winner !== "1" ) {
                        // We are searching for p2 winner, but in reality we
                        // want p1 to be a winner
                        return false;
                    }
                }
                // Sides don't matter for draws
            ...
            if ( p1 ) {
                ...
                if ( sq.winner === "1" && mData.winner !== "2" ) {
                    // We are searching for p1 winner, but in reality we want p2
                    // to be a winner
                    return false;
                } else if ( sq.winner === "2" && mData.winner !== "1" ) {
                    // We are searching for p2 winner, but in reality we want p1
                    // to be a winner
                    return false;
                } else if ( sq.winner === "D" && mData.winner !== "D" ) {
                    // Sides don't matter for draws
        ...
        // Only p2 search
        let p2 =
            playerMatches( mData.p2char, mData.p2name, sq.p2char, sq.p2name );
        let p2asp1 =
            playerMatches( mData.p1char, mData.p1name, sq.p2char, sq.p2name );
        if ( p2 ) {
            // Winner check
            if ( sq.winner === "1" && mData.winner !== "1" ) {
                // Only p1 winner allowed, no p2 or draw
            ...
            // Winner check reversed
            if ( sq.winner === "1" && mData.winner !== "2" ) {
                // We are searching for p1 winner, but in reality we want p2 to
                // be a winner
                return false;
            } else if ( sq.winner === "2" && mData.winner !== "1" ) {
                // We are searching for p2 winner, but in reality we want p1 to
                // be a winner
                return false;
            } else if ( sq.winner === "D" && mData.winner !== "D" ) {
                // Sides don't matter for draws
