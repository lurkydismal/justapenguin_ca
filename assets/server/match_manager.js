...
MatchManger.prototype.validateAddEditVideoForm = function( form ) {
    ...
    if ( !form.tags || typeof form.tags !== "string" ) {
        ...
        form.tags =
            form.tags.replace( /\|+/g, "|" ).split( "|" ).map( x => x.trim() );
    }
};
...
MatchManger.prototype.addMatch = function( form, ip, auth ) {
    ...
    videoData.matches.sort(
        ( a, b ) => { return parseInt( a.time ) - parseInt( b.time ); } );
...
MatchManger.prototype.editMatch = function( form, ip, auth ) {
    ...
    let match = videoData.matches.find( m => parseInt( m.orderIndex ) ===
                                             parseInt( form.orderIndex ) );
    if ( !match ) {
        return;
    }
...
MatchManger.prototype.editVideoSpecificFields = function( videoData, form, ip,
                                                          auth ) {
...
MatchManger.prototype.validateAddEditMatchForm = function(
    form, skipCloseTimeCheck ) {
    if ( parseInt( form.time ).toString() !== form.time.toString() ||
