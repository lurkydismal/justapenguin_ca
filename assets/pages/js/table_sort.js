        ...
        rows.sort( ( a, b ) => {
            if ( config[ idx ] === "text" ) {
                ...
                if ( parseInt( table.dataset.sorted ) === idx ) {
                    return a.cells[ idx ].innerText.localeCompare(
                        b.cells[ idx ].innerText );
                } else {
                    return b.cells[ idx ].innerText.localeCompare(
                        a.cells[ idx ].innerText );
                }
            } else if ( config[ idx ] === "num" ) {
                // Numerical
                if ( parseInt( table.dataset.sorted ) === idx ) {
                    return parseFloat( a.cells[ idx ].innerText ) -
                           parseFloat( b.cells[ idx ].innerText );
                } else {
                    return parseFloat( b.cells[ idx ].innerText ) -
